...
void MyTcpSocket::recvFromSocket()
{
    ...
    */
    ...
    {
        qDebug() << "received data is incomplete...";
        return;
    }
    ...
    else
    {
        ...
        if ((quint64)data_size + 1 + MSG_HEADER <= hasrecvive) //收够一个包
        {
            if (recvbuf[0] == '$' && recvbuf[MSG_HEADER + data_size] == '#') //且包结构正确
            {
                ...
                msgtype = static_cast<MSG_TYPE>(type);
					...
					if (msgtype == CREATE_MEETING_RESPONSE)
					{
						...
						else
						{
							...
							else
							{
								...
								msg->len = data_size;
								queue_recv.push_msg(msg);
                                qDebug()<<"create meeting response...";
							}
						...
						else
						{
							...
							else
							{
								...
								for (int i = 0; i < data_size / sizeof(uint32_t); i++)
								{
                                    ...
                                    qFromBigEndian<uint32_t>(recvbuf + MSG_HEADER + pos, len, &ip);
                                    memcpy_s(msg->data + pos, data_size - pos, &ip, len);
                                    pos += len;
								}
								queue_recv.push_msg(msg);
							}
